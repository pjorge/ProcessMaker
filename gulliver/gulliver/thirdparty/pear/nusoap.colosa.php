	...
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded'){
		...
		switch(true) {
			...
			case (is_int($val) || is_long($val) || $type == 'int'):
				if ($use == 'literal') {
					$xml .= "<$name$xmlns $atts>$val</$name>";
//					$xml .= "<$name$xmlns xsi:type=\"xsd:long\">$val</$name>";
				} else {
					$xml .= "<$name$xmlns xsi:type=\"xsd:int\">$val</$name>";
				}
				break;
...
* @access public
*/
class soap_fault_colosa extends nusoap_base_colosa {
	/**
	...
	*/
	function soap_fault_colosa($faultcode,$faultactor='',$faultstring='',$faultdetail=''){
		parent::nusoap_base_colosa();
...
* @access   public
*/
class XMLSchema extends nusoap_base_colosa  {
...
* @access   public
*/
class soapval extends nusoap_base_colosa {
	/**
...
* @access public
*/
class soap_transport_http extends nusoap_base_colosa {
...
* @access   public
*/
class soap_server_colosa extends nusoap_base_colosa {
	/**
	...
	* @access   public
	*/
	function soap_server_colosa($wsdl=false){
		parent::nusoap_base_colosa();
		// turn on debugging?
		global $debug;
	...
	*/
    function parseRequest($headers, $data) {
		...
		$parser = new soap_parser_colosa($data,$this->xml_encoding,'',$this->decode_utf8);
		// parser debug
	...
	*/
	function fault($faultcode,$faultstring,$faultactor='',$faultdetail=''){
		...
		$this->fault = new soap_fault_colosa($faultcode,$faultactor,$faultstring,$faultdetail);
...
* @access public 
*/
class wsdl extends nusoap_base_colosa {
	...
	function serialize($debug = 0)
	{
		...
		if (count($this->ports) >= 1) {
			foreach($this->ports as $pName => $attrs) {
				...
				$location = 'http://' . getenv ( 'SERVER_NAME' ) . $uri[0] ;
   			$xml .= '<soap:address location="' . $location . ($debug ? '?debug=1' : '') . '"/>';
...
* @access   public
*/
class soap_parser_colosa extends nusoap_base_colosa {
	...
	* @access   public
	*/
	function soap_parser_colosa($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		parent::nusoap_base_colosa();
		$this->xml = $xml;
...
* @access   public
*/
class soapclient_colosa extends nusoap_base_colosa  {
	...
	* @access   public
	*/
	function soapclient_colosa($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30){
		parent::nusoap_base_colosa();
...
/*
		...
		*/
		...
		$this->debug('Use encoding: ' . $this->xml_encoding . ' when creating soap_parser');
		$parser = new soap_parser_colosa($data,$this->xml_encoding,$this->operation,$this->decode_utf8);
